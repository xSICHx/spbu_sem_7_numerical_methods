...
//    static ArrayList<String> rho_k = new ArrayList<>();
    ...
    public static void main(String[] args) {
        ...
        System.out.println("3. Number of iterations m >= " + ln(1/eps)/(sqrt(ksi)));

        System.out.println("4. Spectral radius: " + (omegaOpt-1));

        System.out.println("5. omega: " + omegaOpt);
    ...
    static void doIteration(){
        for (int i = 1; i < N; i++) {
            for (int j = 1; j < M; j++) {
                double us =
                        ( (U[i-1][j] + UPrev[i+1][j])/(hx*hx)  +  (U[i][j-1] + UPrev[i][j+1])/(hy*hy)  + F[i][j] ) /
                                (2/(hx*hx)+2/(hy*hy));
                U[i][j] = UPrev[i][j] + omegaOpt*(us - UPrev[i][j]);
            }
        }
    }
